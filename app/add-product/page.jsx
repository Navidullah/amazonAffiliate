...
export default function ProductAddForm({ onSubmit }) {
  ...
  function handleTypeChange(value) {
    ...
    // Reset image state when switching between own and affiliate products.
    // If the user selects an affiliate product type we should clear any
    // previously chosen file and ensure the image URL field is empty.
    if (value === "affiliate") {
      setImageFile(null);
      setForm((prev) => ({
        ...prev,
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      // Upload image only for own products.  For affiliate items we expect
      // the user to provide a direct URL instead of uploading a file.
      if (type === "own" && imageFile) {
        toast.info("Uploading image...");
        imageUrl = await uploadImageToFirebase(imageFile);
      }
