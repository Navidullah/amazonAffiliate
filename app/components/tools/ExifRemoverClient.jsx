// app/components/tools/ExifRemoverClient.jsx
"use client";
...
export default function ExifRemoverClient() {
  ...
  const previewUrl = useMemo(
    () => (previewUrlFile ? URL.createObjectURL(previewUrlFile) : null),
    [previewUrlFile]
  );
  const cleanUrl = useMemo(
    () => (cleanBlob ? URL.createObjectURL(cleanBlob) : null),
    [cleanBlob]
  );
  useEffect(() => {
    return () => {
  ...
  return (
    <section className="min-h-[60vh] max-w-5xl mx-auto px-4">
      <div
        ...
        aria-label="Upload an image to remove EXIF metadata"
      >
        <input
          ref={inputRef}
          type="file"
...
/** Canvas re-encode removes metadata; respects common EXIF orientation */
