...
export default function ProductDetailsClient({
  ...
  const [qty, setQty] = useState(1);
  const { addToCart } = useCart();
  // Support multiple images: default to array with single image if none provided
  const images =
      ...
      <div className="flex flex-col md:flex-row gap-10 w-full max-w-5xl mx-auto">
            ...
            <CardContent className="p-0 py-0 flex flex-col items-center">
              <div className="relative bg-background shadow-md w-full flex items-center justify-center p-2">
                <Image
                  ...
                  height={0}
                  sizes="100vw"
                  className="w-full h-auto max-h-[400px] object-contain transition-all"
                  priority
                />
                {/* Zoom button opens a larger view */}
                <Sheet>
                  <SheetTrigger asChild>
                    <button className="absolute bottom-2 right-2 bg-black/60 hover:bg-black/80 text-white rounded-full p-2 shadow transition">
                      <ZoomIn size={20} />
            ...
            <div className="grid grid-cols-4 gap-2 w-full max-w-sm">
              {images.map((img, idx) => (
                <button
                  ...
                  className={`border rounded p-1 transition hover:border-primary ${selectedImageIndex === idx ? "border-primary" : "border-transparent"}`}
                >
                  <Image
                    src={img}
        ...
        <div className="w-full md:w-1/2 flex flex-col gap-4 max-w-xl">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {/* Stock for local products */}
            {!isAffiliate && product.stock !== undefined && (
              <Badge
                variant="outline"
                className={`text-xs ${
                  product.stock > 0
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            {relatedProducts.map((item) => (
              <Link key={item.slug} href={`/products/${item.slug}`}>
                <Card className="shadow-md border-0 hover:shadow-lg transition overflow-hidden">
                  <CardContent className="p-4 flex flex-col items-start">
                    ...
                    {item.type === "affiliate" && item.affiliate?.price ? (
