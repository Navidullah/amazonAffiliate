...
import clientPromise from "@/lib/mongodb-adapter";
import User from "@/lib/models/User";
import PinterestAuth from "@/lib/models/PinterestAuth";
import { ConnectToDB } from "@/lib/db";
import bcrypt from "bcrypt";
  ...
  callbacks: {
    async jwt({ token, user, account }) {
      ...
      if (account?.provider === "pinterest") {
        ...
        // Persist to DB (upsert singleton doc) so the pin-posting CLI
        // script can use the token without a live browser session.
        await ConnectToDB();
        await PinterestAuth.findByIdAndUpdate(
          "main",
          {
