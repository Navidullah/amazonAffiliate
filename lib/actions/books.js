...
/** Fetch active books for one curriculum (used by /books/[curriculum] landing pages). */
export const getActiveBooksByCurriculum = async (curriculum) => {
  await ConnectToDB();
  const books = await Book.find({ active: true, curriculum }).sort({ createdAt: -1 }).lean();
  return normalize(books);
};
